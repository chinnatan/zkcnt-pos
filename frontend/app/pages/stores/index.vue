  ...
  <div>
    <NuxtLayout name="auth">
      <UiCraftCard variant="paper" padding="md">
        <h2 class="mb-2 text-center text-xl font-semibold text-ink">{{ t('storesPage.title') }}</h2>
        <p class="mb-6 text-center text-sm text-ink-muted">{{ t('storesPage.subtitle') }}</p>
        ...
        <div v-else-if="storesFetchError" class="py-6 text-center">
          <p class="mb-2 text-ink">{{ t('storesPage.loadError') }}</p>
          <p class="mb-4 text-sm text-danger-500">{{ storeErrorMessage }}</p>
          <button
        ...
        <div v-else-if="userStores.length === 0" class="py-6 text-center">
          <p class="mb-4 text-ink-muted">{{ t('storesPage.noStores') }}</p>
          <button
            class="rounded-lg bg-primary-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-primary-700"
        ...
        <div v-else class="space-y-2">
          <button
            ...
            :key="store.id"
            class="flex w-full items-center gap-3 rounded-lg border border-border-warm p-4 text-left transition-all hover:border-primary-300 hover:bg-primary-50"
          ...
          >
            ...
            <div class="min-w-0 flex-1">
              <div class="font-medium text-ink">{{ store.name }}</div>
              <div class="truncate text-sm text-ink-muted">{{ store.address || t('common.noAddress') }}</div>
            </div>
          ...
          <button
            class="flex w-full items-center justify-center gap-2 rounded-lg border-2 border-dashed border-border-warm p-4 text-sm font-medium text-ink-muted hover:border-primary-300 hover:text-primary-600"
            @click="showCreateModal = true"
          >
    ...
    <Teleport to="body">
      <div v-if="showCreateModal" class="craft-modal-backdrop craft-modal-backdrop--center z-50">
        <div class="craft-modal-panel craft-modal--stitched max-w-md">
          <h3 class="mb-4 text-lg font-semibold text-ink">{{ t('storesPage.createTitle') }}</h3>

          <div v-if="createError" class="mb-4 rounded-lg bg-danger-50 p-3 text-sm text-danger-700">
            {{ createError }}
          </div>
          ...
          <form @submit.prevent="handleCreateStore" class="space-y-4">
            ...
            <div>
              <label class="mb-1 block text-sm font-medium text-ink">{{ t('storesPage.slug') }}</label>
              <input
