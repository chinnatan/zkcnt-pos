    ...
    <template v-else-if="data">
      ...
      <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div class="craft-card craft-card--kraft p-5">
          ...
          <p class="mt-1 text-2xl font-bold text-ink">{{ formatCurrency(data.summary.netSales) }}</p>
        </div>
      </div>
      ...
      <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div class="craft-card craft-card--label p-5">
          <p class="text-sm text-ink-muted">{{ t('reportsPage.voidRefund') }}</p>
      ...
      <div class="grid gap-4 sm:grid-cols-3">
        ...
        <div class="craft-card craft-card--kraft p-5">
          <p class="text-sm text-ink-muted">{{ t('reportsPage.registeredCustomers') }}</p>
      ...
      <div class="rounded-xl bg-paper shadow-sm">
        ...
        <div class="p-5">
          ...
          <div v-if="activeTab === 'products'">
            <div class="mb-4 flex gap-2">
              <button
                ...
                :class="productSort === 'revenue' ? 'bg-primary-100 text-primary-700' : 'bg-surface text-ink-muted'"
              ...
              <button
                type="button"
                class="rounded-lg px-3 py-1.5 text-xs font-medium"
                :class="productSort === 'qty' ? 'bg-primary-100 text-primary-700' : 'bg-surface text-ink-muted'"
                @click="productSort = 'qty'"
              >
                {{ t('reportsPage.byQuantity') }}
              </button>
            </div>
            <div v-if="topProducts.length === 0" class="py-6 text-center text-ink-muted">
            ...
            <div v-else class="space-y-2">
              ...
              >
                ...
                <span class="text-sm text-ink-muted">{{ t('common.sold', { qty: tp.qty }) }}</span>
                <span class="text-sm font-semibold">{{ formatCurrency(tp.revenue) }}</span>
                <span class="text-xs text-accent-600">{{ formatCurrency(tp.margin) }}</span>
              </div>
            </div>
          </div>
          ...
          <div v-else-if="activeTab === 'customers'">
            <div v-if="data.topCustomers.length === 0" class="py-6 text-center text-ink-muted">
              {{ t('reportsPage.noData') }}
            </div>
            <div v-else class="space-y-2">
              <div
